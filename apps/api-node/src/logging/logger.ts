import type { DiagnosticsLogStore } from "./diagnostics.js";
...
export function createJsonLogger(options: JsonLoggerOptions): Logger {
  ...
  function write(
    ...
    options.diagnostics?.append(entry);
...
export function sanitizeRequestUrl(requestUrl: string | undefined): string {
  if (!requestUrl) {
    return "/";
  }
...
function sanitizeValue(
  ...
  if (typeof value === "string") {
    if (
      key &&
      (key === "url" || key === "path" || key.toLowerCase().endsWith("url"))
    ) {
