...
export async function runEmailEngineRealWebhookSmoke(
  ...
  const delivery = await sendMessage({
    ...
    to: onboarding.email,
    messageId,
    subject: `[EmailHub Real Webhook Smoke] ${uniqueId}`,
    text: [
  ...
  const diagnostic = await waitForMessageUpsertedDiagnostic({
    apiBaseUrl,
    fetchImpl,
    accountId: onboarding.accountId,
  ...
  return {
    ...
    ...(diagnostic.jobId ?? readString(diagnostic.context.syncJobId)
      ? { webhookSyncJobId: diagnostic.jobId ?? readString(diagnostic.context.syncJobId) }
      : {}),
  };
}

async function waitForMessageUpsertedDiagnostic(input: {
  apiBaseUrl: string;
  fetchImpl: typeof fetch;
  accountId: string;
  notBefore: string;
  ...
  throw new Error(
    `EmailEngine real webhook smoke did not observe message_upserted for ${input.accountId} after ${attempts} diagnostics polls`,
  );
}
